...
import { motion, useMotionValue, useAnimate, stagger } from "framer-motion";
 ...
 */ export default function MotionText(props) {
	const { opacity, x, y, scale, rotate, rotateX, rotateY } =
		props.effectOptions;
	...
	function getSplitTextProps({ isLetter, isWord, isLine }) {
		...
		if (isLetter)
			return {
				LetterWrapper: (info) =>
					/*#__PURE__*/ _jsx(MotionSpanComponent, {
						...info,
						...sharedProps,
					}),
			};
	...
	const [scope, animate] = useAnimate(); // If the variant changes, trigger the animation if the variant boolean has changed.
	useEffect(() => {
		if (isCanvas) return;
		let animation;
		const requestId = requestAnimationFrame(() => {
			...
			animation = animate(
				".motion-text-segment",
				props.variant ? item.visible : item.hidden,
				{
					...filteredTransition,
					delay: stagger(props.stagger, { startDelay: delay }),
				}
			);
	...
	return /*#__PURE__*/ _jsx(motion.div, {
		...
		initial: isCanvas || hasAnimated ? "visible" : "hidden",
		children: isCanvas
			? props.text
